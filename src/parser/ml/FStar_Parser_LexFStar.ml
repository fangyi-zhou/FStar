...
match%sedlex lexbuf with
 ...
 | "(*", Star '*', "*)" -> token lexbuf (* avoid confusion with fsdoc *)
 | "(**" -> fsdoc (1,"",[]) lexbuf
...
match%sedlex lexbuf with
 | '\\', newline, Star anywhite -> L.new_line lexbuf; string buffer lexbuf
 | newline ->
...
match%sedlex lexbuf with
 ...
 | Star anywhite, "@", Plus ('a' .. 'z' | 'A' .. 'Z'), Opt ':', Star anywhite ->
...
and fsdoc_kw_arg (n, doc, kw, kwn, kwa) lexbuf =
match%sedlex lexbuf with
 | newline ->
   L.new_line lexbuf;
